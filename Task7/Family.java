...
public class Family extends Person implements ShelfUser {
    ...
    private void extract1(StringBuilder sb, ArrayList<Family> brothers) {
        if (brothers.size() > 0) {
            ...
            for (Family brother : brothers) {
                sb.append(brother.getFio()).append(", ");
            }
    ...
    @Override
    public void placeOnShelf(int shelfNumber, CabinetOperations cabinetOperations) {
        if (!cabinetOperations.isOpen()) {
            System.out.println("Шкаф закрыт, нельзя положить на полку.");
        } else {
            System.out.println(fio + " положил(а) что-то на полку " + shelfNumber);
        }
    }
}